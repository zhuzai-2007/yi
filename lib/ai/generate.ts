import { AI_PROMPT_VERSION, SYSTEM_PROMPT, META_REPAIR_PROMPT } from "./prompt";
  ...
  return [
    {
      ...
      content:
        SYSTEM_PROMPT +
        "\n输出 JSON Schema：\n" +
        JSON.stringify(interpretationJsonSchema(context.facts.has_changes)),
    },
    {
      role: "user",
      content:
...
export async function generateInterpretation(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const raw = await transport.generate({
      messages,
      signal,
      hasChanges: context.facts.has_changes,
    });
    if (signal.aborted) throw new AiError("abort");
    onStage("正在校验返回结果……");
    try {
